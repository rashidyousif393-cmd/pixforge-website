...
export default function PainPoints() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(resultRef.current, { opacity: 0, scale: 0.95 }, { opacity: 1, scale: 1, duration: 0.5, ease: "power2.out" });
    });
    return () => ctx.revert();
  }, [showResult]);
  ...
  const sectionRef = useGsapAnimation<HTMLElement>((scope, { reducedMotion }) => {
    ...
    const copy = scope.current.querySelector('[data-anim="copy"]');
    const points = scope.current.querySelectorAll('[data-anim="point"]');
    const panel = scope.current.querySelector('[data-anim="panel"]');

    const tl = gsap.timeline({
      defaults: { ease: "power3.out" },
      scrollTrigger: { trigger: scope.current, start: "top 80%", toggleActions: "play none none none" },
    });
    if (badge) tl.from(badge, { opacity: 0, y: -14, duration: 0.6 });
    if (heading) tl.from(heading, { opacity: 0, y: 24, duration: 0.7 }, "-=0.4");
    if (copy) tl.from(copy, { opacity: 0, y: 18, duration: 0.6 }, "-=0.45");
    if (points.length) tl.from(Array.from(points), { opacity: 0, y: 16, duration: 0.5, stagger: 0.12 }, "-=0.35");
    if (panel) tl.from(panel, { opacity: 0, y: 24, scale: 0.97, duration: 0.7 }, "-=0.5");
  }, []);

  return (
    <section id="diagnostica" ref={sectionRef} className="py-20 sm:py-24 lg:py-32 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-6 space-y-6">
            <div data-anim="badge" className="inline-flex items-center gap-2 text-gold-500 bg-gold-950/20 border border-gold-900/40 rounded-full px-3.5 py-1 text-xs font-mono">
            ...
            <p data-anim="copy" className="font-sans text-base sm:text-lg text-zinc-300 leading-relaxed font-light">
              {language === "it"
            ...
            <div className="space-y-4 pt-4">
              <div data-anim="point" className="flex items-start gap-3">
          ...
          <div className="lg:col-span-6">
            <div data-anim="panel" className="glass-panel rounded-2xl p-6 sm:p-8 border border-white/5 shadow-2xl relative overflow-hidden">
