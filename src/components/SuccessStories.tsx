...
export default function SuccessStories() {
  ...
  const sectionRef = useGsapAnimation<HTMLElement>((scope, { reducedMotion }) => {
    ...
    const badge = scope.current.querySelector('[data-anim="badge"]');
    const heading = scope.current.querySelector('[data-anim="heading"]');
    const subtitle = scope.current.querySelector('[data-anim="subtitle"]');
    const cards = scope.current.querySelectorAll('[data-anim="article-card"]');
    const banner = scope.current.querySelector('[data-anim="banner"]');

    const headerTl = gsap.timeline({
      defaults: { ease: "power3.out" },
      scrollTrigger: { trigger: scope.current, start: "top 85%", toggleActions: "play none none none" },
    });
    if (badge) headerTl.from(badge, { opacity: 0, y: 16, duration: 0.6 });
    if (heading) headerTl.from(heading, { opacity: 0, y: 18, duration: 0.7 }, "-=0.4");
    if (subtitle) headerTl.from(subtitle, { opacity: 0, y: 16, duration: 0.6 }, "-=0.45");

    // Staggered entrance for each editorial card as it scrolls into view
  ...
  return (
    <section id="storie-successo" ref={sectionRef} className="py-20 sm:py-32 relative overflow-hidden w-full">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 relative z-10 w-full">
        ...
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto section-header space-y-5">
          <div data-anim="badge" className="kicker kicker--blue justify-center">
            {language === "it" ? "STORIE DI SUCCESSO" : "SUCCESS STORIES & STRATEGIES"}
          </div>
          <h2 data-anim="heading" className="section-title">
            {language === "it" ? (
              <>
          ...
          <p data-anim="subtitle" className="text-zinc-400 text-sm sm:text-base leading-relaxed font-light">
            {language === "it"
        ...
        <div className="space-y-28">
          {articles.map((article, index) => {
            ...
            return (
              <div
                key={article.id}
                data-anim="article-card"
                className="card-glow-hover group grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center text-left transition-shadow duration-500 hover:shadow-2xl hover:shadow-gold-500/5 rounded-3xl"
              >
                {/* Visual / Image Side */}
                <div className={`col-span-1 lg:col-span-6 relative ${isEven ? "lg:order-1" : "lg:order-2"} group w-full`}>
                  ...
                  <ScrollReveal effect="zoom" className="relative rounded-3xl overflow-hidden border border-white/5 bg-zinc-950 aspect-[16/10] shadow-2xl w-full">
                ...
                <div className={`col-span-1 lg:col-span-6 space-y-6 text-left ${isEven ? "lg:order-2" : "lg:order-1"} w-full`}>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-[10px] font-mono font-bold tracking-wider text-zinc-400 uppercase">
        ...
        <div data-anim="banner" className="mt-32 relative rounded-3xl border border-zinc-800 bg-zinc-950/40 p-8 sm:p-12 text-center overflow-hidden w-full">
