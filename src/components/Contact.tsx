...
export default function Contact() {
  ...
  const magneticSubmitRef = useMagneticEffect<HTMLButtonElement>(0.2);

  // Section-wide reveal: a soft "brightening" mood-glow wash scrubs in as the section
  // enters view, plus a staggered banner intro and a progressive form-field entrance.
  const contactRef = useGsapAnimation<HTMLElement>((scope, { reducedMotion }) => {
    if (reducedMotion || !scope.current) return;
    const el = scope.current;

    const moodGlow = el.querySelector('[data-anim="mood-glow"]');
    if (moodGlow) {
      gsap.set(moodGlow, { opacity: 0 });
      gsap.to(moodGlow, {
        opacity: 1,
        ease: "none",
        scrollTrigger: {
    ...
    const bannerBadge = el.querySelector('[data-anim="banner-badge"]');
    const bannerTitle = el.querySelector('[data-anim="banner-title"]');
    const bannerSubtitle = el.querySelector('[data-anim="banner-subtitle"]');
    const bannerCta = el.querySelector('[data-anim="banner-cta"]');
    if (bannerBadge || bannerTitle || bannerSubtitle || bannerCta) {
      const bannerTl = gsap.timeline({
        defaults: { ease: "power3.out" },
        scrollTrigger: { trigger: el, start: "top 85%", toggleActions: "play none none none" },
      });
      if (bannerBadge) bannerTl.from(bannerBadge, { opacity: 0, y: 15, duration: 0.6 });
      if (bannerTitle) bannerTl.from(bannerTitle, { opacity: 0, y: 15, duration: 0.6 }, "-=0.4");
      if (bannerSubtitle) bannerTl.from(bannerSubtitle, { opacity: 0, y: 15, duration: 0.6 }, "-=0.4");
      if (bannerCta) bannerTl.from(bannerCta, { opacity: 0, y: 10, scale: 0.97, duration: 0.6 }, "-=0.35");
    }

    const formContainer = el.querySelector("#contact-form-anchor");
    const formFields = el.querySelectorAll('[data-anim="form-field"]');
  ...
  // Brief entrance for the success confirmation panel once the submission succeeds.
  const successPanelRef = useGsapAnimation<HTMLDivElement>((scope, { reducedMotion }) => {
    if (reducedMotion || !scope.current) return;
    gsap.from(scope.current, { opacity: 0, scale: 0.95, duration: 0.5, ease: "power3.out" });
  }, [status]);

  return (
    <section id="contatti" ref={contactRef} className="py-24 sm:py-32 lg:py-40 relative overflow-hidden w-full">
      ...
      {/* Mood-shift wash: brightens subtly as the section scrolls into view, giving a
          dark-to-light emotional lift without breaking the dark aesthetic. */}
      <div data-anim="mood-glow" className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_35%,rgba(223,181,28,0.07),rgba(0,122,255,0.04)_55%,transparent_75%)] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 relative z-10 w-full">
        ...
        <div className="relative rounded-3xl border border-zinc-900 bg-gradient-to-b from-zinc-950/80 to-zinc-950/40 p-8 sm:p-12 lg:p-16 text-center overflow-hidden mb-24 w-full shadow-2xl backdrop-blur-xl">
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            <div
              data-anim="banner-badge"
              className="kicker justify-center mx-auto"
            >
              {language === "it" ? "Studio di Eccellenza Digitale Svizzera" : "Swiss Digital Excellence Studio"}
            </div>

            <h2
              data-anim="banner-title"
        ...
        <div id="contact-form-anchor" className="grid grid-cols-1 lg:grid-cols-12 gap-12 sm:gap-16 items-start scroll-mt-28 text-left w-full">
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-zinc-950/40 border border-zinc-900 rounded-2xl text-zinc-400 text-[11px] w-full">
                ...
                {status === "success" ? (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {/* Name */}
                        <div data-anim="form-field" className="space-y-1.5 text-left">
                      ...
                      <div className="flex flex-col sm:flex-row gap-4 pt-3">
                        <button
                          ...
                          className="btn-primary flex-1 !py-4 !px-6 !rounded-xl text-xs tracking-wider uppercase disabled:opacity-40"
                        >
                          {status === "submitting" ? (
                            <>
