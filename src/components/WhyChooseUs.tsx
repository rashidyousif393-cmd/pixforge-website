...
export default function WhyChooseUs() {
  ...
  // Refs for the drag-free before/after showcase panels (tab-toggle only, no pointer drag here)
  const beforeAfterListRef = useRef<HTMLDivElement>(null);
  ...
  // Badge/heading/subtitle/pillar-card entrances now run through <ScrollReveal>
  // (IntersectionObserver + CSS) -- see the JSX below.

  // Elegant fade for the before/after tab-switch content. This is a plain state
  // toggle (buttons), not the drag mechanism, so it's safe to animate directly.
  useEffect(() => {
    const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reducedMotion) return;
    if (beforeAfterListRef.current) {
      gsap.fromTo(
        beforeAfterListRef.current,
        { opacity: 0, x: activeTab === "before" ? -10 : 10 },
  ...
  return (
    <section id="perche-sceglierci" className="py-20 sm:py-32 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Pillars Grid -- an oversized ghost numeral anchors each card instead of the
            generic "icon chip + title" template, giving this section its own identity
            distinct from the icon-led Services grid earlier on the page. */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-zinc-900/80 border border-zinc-900/80 rounded-2xl overflow-hidden mb-24">
          {pillars.map((pillar, index) => {
            ...
            return (
              <ScrollReveal
                ...
                effect="fade-up"
                delay={(index % 3) * 90}
                className="card-glow-hover group relative bg-zinc-950 hover:bg-zinc-900/60 p-7 sm:p-8 transition-colors duration-500 text-left"
              >
                <span className="font-display font-extrabold text-5xl text-white/[0.06] group-hover:text-gold-500/10 leading-none select-none transition-colors duration-500">
                ...
                <p className="mt-3 text-xs sm:text-sm text-zinc-400 leading-relaxed font-light">
                  {pillar.description}
                </p>
              </ScrollReveal>
            );
        ...
        <div className="rounded-3xl border border-zinc-800/80 bg-zinc-900/20 backdrop-blur-md p-6 sm:p-10 lg:p-12 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-5 space-y-6 text-left">
              ...
              <div className="space-y-4 min-h-[160px]" ref={beforeAfterListRef}>
                {activeTab === "before" ? (
                  <div
                    key="before-list"
                    className="space-y-3"
            ...
            <div className="lg:col-span-7 flex justify-center" ref={visualCardRef}>
              {activeTab === "before" ? (
                  <div
                    key="before-card"
                  ...
                  >
                    ...
                    <div className="space-y-4 text-zinc-600">
                      ...
                      <div className="border border-zinc-900/80 rounded-xl p-4 flex justify-between items-center text-zinc-400 bg-zinc-900/20">
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between text-xs text-zinc-400">
                        <span className="font-mono text-[10px] text-zinc-400 uppercase">{language === "it" ? "AURA BEAUTY SPA LUGANO" : "AURA BEAUTY SPA ZURICH"}</span>
                      ...
                      <div className="pt-2 border-t border-zinc-800 flex justify-between items-center text-zinc-400 text-xs">
                        <div className="text-center flex-1 border-r border-zinc-800">
                          ...
                          <div className="text-[8px] text-zinc-400 uppercase tracking-wider">{language === "it" ? "Fatturato" : "Revenue"}</div>
                        </div>
                        <div className="text-center flex-1 border-r border-zinc-800">
                          <div className="font-mono font-extrabold text-emerald-400 text-lg">100/100</div>
                          <div className="text-[8px] text-zinc-400 uppercase tracking-wider">SEO Score</div>
                        </div>
                        <div className="text-center flex-1">
                          <div className="font-mono font-extrabold text-indigo-400 text-lg">24/7</div>
                          <div className="text-[8px] text-zinc-400 uppercase tracking-wider">{language === "it" ? "Contatti" : "Inquiries"}</div>
                        </div>
                      </div>
                    </div>
                  </div>
