...
import {
  ArrowUp, Mail, MapPin, MessageCircle,
  ClipboardCheck, ClipboardCopy, Instagram, Facebook,
  ArrowRight, Lock, Cookie
} from "lucide-react";
...
export default function Footer() {
  ...
  // Anchor links (#chi-siamo, #contatti, ...) only make sense on the homepage: if
  // we're on another route (e.g. /blog), navigate to "/" with the hash instead --
  // HomePage's own hash-scroll effect picks it up once it mounts -- rather than
  // silently doing nothing, which was the previous behaviour for every footer
  // anchor link when viewed from a non-homepage page.
  ...
  // Premium closing bookend: a brief glow-pulse + fade/scale reveal on the footer's
  // logo mark and wordmark once it scrolls into view, echoing the intro overlay's
  // opening logo moment but shorter and simpler.
  const footerRef = useGsapAnimation<HTMLElement>((scope, { reducedMotion }) => {
    if (reducedMotion || !scope.current) return;
    const el = scope.current;
    const logoMark = el.querySelector('[data-anim="footer-logo-mark"]');
    const logoWord = el.querySelector('[data-anim="footer-logo-word"]');
    if (!logoMark && !logoWord) return;
    ...
    if (logoMark) {
      tl.fromTo(
        logoMark,
        { opacity: 0, scale: 0.7, filter: "drop-shadow(0 0 0px rgba(223,181,28,0))" },
        {
          opacity: 1,
          scale: 1,
          filter: "drop-shadow(0 0 16px rgba(223,181,28,0.6))",
          duration: 0.5,
          ease: "power3.out",
        },
        0
      );
      tl.to(logoMark, { filter: "drop-shadow(0 0 0px rgba(223,181,28,0))", duration: 0.4, ease: "power2.out" }, "-=0.05");
    }
    if (logoWord) {
      tl.from(logoWord, { opacity: 0, y: 10, duration: 0.45, ease: "power3.out" }, logoMark ? "-=0.5" : 0);
    }
  }, []);

  return (
    <footer id="agency-footer" ref={footerRef} className="border-t border-zinc-900 pt-20 pb-10 relative overflow-hidden w-full group">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8 relative z-10 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 pb-16 border-b border-zinc-900 text-left w-full">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="flex items-center gap-3">
              <div data-anim="footer-logo-mark" className="w-10 h-10 rounded-xl bg-gradient-to-br from-gold-500 to-electric-blue flex items-center justify-center p-[1px] shadow-lg shadow-gold-500/5">
              ...
              <div data-anim="footer-logo-word" className="flex flex-col">
            ...
            <div className="space-y-2.5 pt-2">
              <h5 className="font-mono text-[10px] font-bold text-zinc-400 uppercase tracking-widest">
                {language === "it" ? "Seguici" : "Follow Us"}
              </h5>
              <div className="flex items-center gap-3">
                {[
                  { icon: <Instagram className="w-4 h-4" />, href: "https://www.instagram.com/pixforge_ticino/", label: "Instagram" },
                  { icon: <Facebook className="w-4 h-4" />, href: "https://www.facebook.com/PixelForgeCH/", label: "Facebook" },
                ].map((social, idx) => (
                  <a
                    key={idx}
                    href={social.href}
                    ...
                    aria-label={
                      language === "it"
                        ? `Seguici su ${social.label} (si apre in una nuova scheda)`
                        : `Follow us on ${social.label} (opens in a new tab)`
                    }
                    className="w-9 h-9 rounded-xl bg-zinc-950 border border-zinc-900 text-zinc-400 hover:text-gold-500 hover:border-gold-500/30 hover:-translate-y-[3px] hover:scale-105 hover:shadow-[0_0_16px_rgba(223,181,28,0.35)] transition-all flex items-center justify-center shadow-md cursor-pointer"
                    title={social.label}
                  >
                    {social.icon}
                  </a>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="block p-3.5 rounded-2xl border border-emerald-500/20 bg-gradient-to-r from-[#075E54]/10 to-[#25D366]/5 hover:from-[#075E54]/20 hover:to-[#25D366]/10 hover:-translate-y-[2px] hover:scale-[1.01] transition-all cursor-pointer relative overflow-hidden group text-left"
              >
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between pt-10 gap-6 w-full">
          ...
          <div className="space-y-1 text-center lg:text-left">
            <p className="font-sans text-[10px] sm:text-xs text-zinc-400 font-light">
              {language === "it"
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            ...
            <div className="px-3 py-1.5 bg-zinc-950 border border-zinc-900 rounded-xl flex items-center gap-2.5">
              ...
              <span className="font-mono text-[9px] text-zinc-400 tracking-wider font-semibold">
            ...
            {/* SSL Secure pill */}
            <div className="px-3 py-1.5 bg-zinc-950/40 border border-zinc-900/60 rounded-xl flex items-center gap-2 text-zinc-400">
          ...
          <div className="flex flex-wrap items-center justify-center gap-6">
            <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-[10px] font-mono text-zinc-400">
              <Link to="/privacy-policy" className="hover:text-gold-500 transition-colors">
                Privacy Policy
              </Link>
              <span>•</span>
              <Link to="/cookie-policy" className="hover:text-gold-500 transition-colors">
                Cookie Policy
              </Link>
              <span>•</span>
              <Link to="/termini-e-condizioni" className="hover:text-gold-500 transition-colors">
              ...
              <button
                ...
                className="hover:text-gold-500 transition-colors cursor-pointer inline-flex items-center gap-1.5"
              >
            ...
            <button
              ...
              className="w-10 h-10 bg-zinc-950 hover:bg-zinc-900 border border-zinc-900 hover:border-gold-500/30 text-zinc-400 hover:text-white hover:scale-105 active:scale-95 rounded-xl transition-all cursor-pointer flex items-center justify-center shadow-lg group"
